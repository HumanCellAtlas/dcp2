...
``Schema Changes for Projects (managed access)``

Managed Access projects have additional data we want to be able to capture. We previously did not have the fields to capture this data in our metadata schema. We will therefore create two new optional fields.

- DbGAP Accession Field

- EGA Accession Field

DUO (Data Use Ontology) codes will not be included at this time.



``Schema changes for CGMs (dcp3)``

For release 3 and onwards, we intend to store matrices provided by data contributors (as opposed to the matrices generated by the DCP) in the informationally rich metadata schema. The CGMs will be described as ‘analysis_file’ entities, similar to DCP-generated matrices, and will be linked via a process to:
the ‘sequence_file’ OR ‘cell suspension’ entities that contributed to it
the ‘analysis_protocol’ that generated the file
the ‘library_preparation_protocol’ applied to the cell suspensions that generated it.
The CGM files will be exported from Ingest as analysis files, and imported and indexed as analysis files.

Four optional fields are added to the schema to facilitate storing CGMs metadata.

- “analysis_file.file_core.file_source”
    - optional field set to one of two values, contributor-generated or DCP-generated.

- “analysis_file.matrix.cell_count”
    - Optional field storing the number of cells which are analyzed in the analysis file

- “analysis_protocol.matrix.data_normalization”
    - Optional descriptive field describing the processes by which the analysis data is normalized
    - Ideally ontologised, but as there is no appropriate ontology yet we are including it as a first step, and will be re-assessed

- “analysis_protocol.matrix.derivation_process”
    - Optional descriptive field describing the computational tools used to generate the analysis file
    - Ideally ontologised, but as there is no appropriate ontology yet we are including it as a first step, and will be re-assessed

Additionally the ‘content_description’ field, an ontologized field, will now be set to one of three options, depending
on which ontology_label best describes the file. Count matrices are unnormalized values. Gene expression matrices are
normalized, and cell type identifier is any file that helps to identify cell types.::

    {
        "text": "Contributor-generated count matrix",
        "ontology": "data:3917",
        "ontology_label": "Count Matrix"
    }

    {
        "text": "Contributor-generated gene expression matrix",
        "ontology": "data:3112",
        "ontology_label": "Gene expression matrix"
    }

    {
        "text": "Contributor-generated cell type identifier files",
        "ontology": "data:2655",
        "ontology_label": "Cell type identifier"
    }

``Subgraph Changes for CGMs (managed access)``

For managed access datasets, raw data and metadata may not be available. Ideally we would link CGMs to the specific sequence files which have created them. However, if the sequence file is not present in the metadata (due to controlled access or the information not being known), the analysis_file would be linked to a biomaterial in the metadata schema.

There is currently one graph shape being tested by Azul and other components as a candidate subgraph, in which cell suspension links directly to analysis file.
Specifically:
Donor —> Specimen —> Cell Suspension —> Analysis File


.. figure:: media/managed-cgms.png
   :align: center



It is important to note here that this means we have not tested, and hence pipelines and Azul may not be able to process or index, subgraphs in which biomaterials other than ‘cell_suspension’ link to the analysis file.

At the moment, no post-processing is performed on contributor-generated matrices. This will be kept the same. Users will be able to download them in exactly the same file format as they were provided by the contributor. For example, if the contributor provided a ZIP archive of multiple matrix files, the user will need to download that archive and extract it locally.

``Stratification``

Stratification can be produced by linking the CGM analysis file to specific sequence_file or biomaterial entities. If there are matrices which are generated from a subset of sequence files, we are able to link only that subset of sequence files to the CGM.

This generates a subgraph from biomaterial to sequence file to analysis file (CGM) per matrix file provided by the contributor. In the case of a ZIP archive of multiple matrix files, the ZIP archive functions as a single analysis_file entity, and would be described by a single subgraph.
