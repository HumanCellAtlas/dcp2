...
- “analysis_protocol.matrix.data_normalization”
    ...
    - Ideally ontologised, but as there is no appropriate ontology yet will be a first step to include in the schema, and will be re-assessed
...
Additionally the ‘content_description’ field, an ontologized field, will now be set to one of three options, depending on which ontology_label best describes the file. Count matrices are unnormalized values. Gene expression matrices are normalized, and cell type identifier is any file that helps to identify cell types.
