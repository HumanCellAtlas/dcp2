...
3) Otherwise, if the request is for a `contributor-generated project-level
   matrix data file <Contributor-generated matrices (CGMs)_>`_, and the request
   is made to the Azul REST API, the request will be satisfied.

4) Otherwise, if the request is accompanied by a valid OAuth 2.0 token, and
...
5) Otherwise, the request will be denied.
